...
const getMistral = () => {
    ...
    if (!apiKey) {
        throw new Error("MISTRAL_API_KEY is not defined in environment variables. Please add it to your .env file.");
    }
    return new Mistral({ apiKey });
};

// Helper to detect and handle Mistral quota/rate-limit errors
const handleAIError = (error, res) => {
    const msg = error.message || '';
    if (error.statusCode === 429 || msg.includes('429') || msg.includes('quota') || msg.includes('Too Many Requests')) {
        return res.status(429).json({
            message: 'The AI service is temporarily unavailable due to a rate limit. Please try again in a minute.',
            retryAfter: 60
        });
    }
